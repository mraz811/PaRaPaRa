...
<div id="container">
	...
	<c:if test="${loginDto.auth eq 'A'}">
		담당자 정보 가져올 곳
		<div>
			<input type="hidden" id="pwchkVal" value="0">
			...
			<form action="./adminModi.do" method="post" onsubmit="return confirmModi()">
				...
				<input type="text" id="email" name="admin_email" placeholder="이메일" required="required" maxlength="80" value="${aDto.admin_email}">
				<br>
	...
	<c:if test="${loginDto.auth eq 'U'}">
		...
		<div>
			<form action="./ownerModi.do" method="post" onsubmit="return confirmModi()">
				...
				<br><span>변경할 새 비밀번호</span><span id="pwRst"></span><br>	
				<input type="password" id="pw" name="owner_pw" placeholder="변경할 비밀번호"  maxlength="12">
				...
				<br><span>이메일</span><span id="emailRst"></span><br>	
				<input type="text" id="email" name="owner_email" placeholder="이메일" required="required" maxlength="80" value="${oDto.owner_email}">
				<br>
				<input type="submit" value="수정하기">
				<input type="button" value="돌아가기" onclick="backToMain()">
			</form>
